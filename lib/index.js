...
AutoSequelize.prototype.run = function(callback) {
  ...
  function generateText(err) {
    ...
    async.each(_.keys(self.tables), function(table, _callback){
      ...
      if( self.options.v5 ) {
        tsTableDef = self.options.typescript ? 'export interface ' + tableName + 'Model extends Sequelize.Model {' : '';
      }
      ...
      if(!self.options.typescript){
        ...
        text[table] = tsHelper.model.getModelFileStart(self.options.indentation, spaces, tableName, self.options.v5);
      }
      ...
      if(self.options.typescript) typescriptFiles[0] += tsHelper.def.getTableDefinition(tsTableDef, tableName, self.options.v5);
      ...
      if(self.options.typescript) typescriptFiles[1] = tsHelper.model.generateTableModels(_.keys(text), self.options.spaces, self.options.indentation, self.options.camelCase, self.options.camelCaseForFileName, self.options.v5);
